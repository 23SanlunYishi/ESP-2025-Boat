...
static uint16_t FIFOCON_REGISTER(const uint16_t inFIFOIndex)
{ // 1 ... 31
...
static uint16_t FIFOSTA_REGISTER(const uint16_t inFIFOIndex)
{ // 1 ... 31
...
static uint16_t FIFOUA_REGISTER(const uint16_t inFIFOIndex)
{ // 1 ... 31
...
static uint16_t FLTCON_REGISTER(const uint16_t inFilterIndex)
{ // 0 ... 31 (DS20005688B, page 58)
  return 0x1D0 + inFilterIndex;
}

// ······················································································································

static uint16_t FLTOBJ_REGISTER(const uint16_t inFilterIndex)
{ // 0 ... 31 (DS20005688B, page 60)
  return 0x1F0 + 8 * inFilterIndex;
}

// ······················································································································

static uint16_t MASK_REGISTER(const uint16_t inFilterIndex)
{ // 0 ... 31 (DS20005688B, page 61)
                           ...
                           const ACAN2517FDFilters &inFilters)
{
  ...
  {
    if (mINT != 255)
    { // 255 means interrupt is not used (thanks to Tyler Lewis)
    ...
    while (wait)
    {
      ...
      { // Wait (2 ms max) until the configuration mode is reached
...
void ACAN2517FD::isr_poll_core(void)
{
  ...
  while (handled)
  {
    ...
    if (mRxInterruptEnabled && ((it & (1 << 1)) != 0))
    { // Receive FIFO interrupt
      receiveInterrupt();
      handled = true;
    }
    if ((it & (1 << 10)) != 0)
    { // Transmit Attempt interrupt
...
{ // Generated if hardware transmit FIFO is not full
  ...
  {                         // No message in transmit FIFO: disable "FIFO not full" interrupt
    uint8_t data8 = 1 << 7; // FIFO is a transmit FIFO
    data8 |= 1 << 4;        // TXATIE ---> 1: Enable Transmit Attempts Exhausted Interrupt
    writeRegister8Assume_SPI_transaction(FIFOCON_REGISTER(TRANSMIT_FIFO_INDEX), data8);
    mHardwareTxFIFOFull = false;
...
bool ACAN2517FD::recoverFromRestrictedOperationMode(void)
{
  ...
  if (currentOperationMode() == ACAN2517FDSettings::RestrictedOperation)
  { // In Restricted Operation Mode
...
uint32_t ACAN2517FD::diagInfos(const int inIndex)
{ // thanks to Flole998 and turmary

...
public:
  ACAN2517FD(const uint8_t inCS,   // CS input of MCP2517FD
...
public:
  ACAN2517FDSettings::OperationMode currentOperationMode(void);

public:
  void setOperationMode(const ACAN2517FDSettings::OperationMode inMode);
...
public:
  bool performSleepModeToConfigurationMode(void);
...
private:
  TaskHandle_t mESP32TaskHandle = nullptr;
#endif
private:
...
public:
  void gpioSetMode(const uint8_t inPin, const uint8_t inMode);

public:
  void gpioWrite(const uint8_t inPin, const uint8_t inLevel);

public:
  bool gpioRead(const uint8_t inPin);

public:
  void configureGPIO0AsXSTBY(void);
